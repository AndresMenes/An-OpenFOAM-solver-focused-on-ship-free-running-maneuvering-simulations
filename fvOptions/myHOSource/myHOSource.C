...
namespace fv
{
    defineTypeNameAndDebug(myHOSource, 0);
    addToRunTimeSelectionTable(option, myHOSource, dictionary);
}
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::fv::myHOSource::myHOSource
(
    const word& name,
...
:
    ...
    diskRPS_(coeffs_.lookupOrDefault<scalar>("diskRPS", 5.0)), // <--- Runtime-modifiable
...
void Foam::fv::myHOSource::update()
{
    // Update diskRPS during runtime
    updateDiskRPS();
...
void Foam::fv::myHOSource::updateDiskRPS()
{
    // Re-fetch the scalar value from coeffs_
    diskRPS_ = coeffs_.lookupOrDefault<scalar>("diskRPS", diskRPS_);

    Info << "Updated diskRPS to: " << diskRPS_ << endl;
}

void Foam::fv::myHOSource::updateCells()
{
