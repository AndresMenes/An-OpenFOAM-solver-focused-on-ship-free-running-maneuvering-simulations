...
void Foam::RBD::rigidBodyMotion::status(const label bodyID) const
{
    ...
    //const spatialVector vvCofR(v(bodyID));
...
const Foam::vector Foam::RBD::rigidBodyMotion::wCofR(const label bodyID) const
{
    const spatialVector velCofR(v(bodyID, Zero));
...
const Foam::tensor Foam::RBD::rigidBodyMotion::tCofR(const label bodyID) const
{
    const spatialTransform CofR(X0(bodyID));
