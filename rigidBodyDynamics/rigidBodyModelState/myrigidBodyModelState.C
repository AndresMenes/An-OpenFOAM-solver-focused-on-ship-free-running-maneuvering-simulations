...
#include "myrigidBodyModelState.H"
...
:
    q_(scalarField(model.nDoF(), Zero)),
    qDot_(scalarField(model.nDoF(), Zero)),
    qDdot_(scalarField(model.nDoF(), Zero)),
    t_(dict.getOrDefault<scalar>("t", -1)),
    deltaT_(dict.getOrDefault<scalar>("deltaT", 0))
{
    const scalarField qT(dict.getOrDefault("q", scalarField((model.countJoint().size()-1)*6, Zero)));
    const scalarField qDotT(dict.getOrDefault("qDot", scalarField((model.countJoint().size()-1)*6, Zero)));
    const scalarField qDdotT(dict.getOrDefault("qDdot", scalarField((model.countJoint().size()-1)*6, Zero)));

    model.readingState(qT, qDotT, qDdotT, q_, qDot_, qDdot_);
